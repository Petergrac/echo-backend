...
export class HashtagService {
  ...
  async processHashtagsForEcho(echoId: string, content: string): Promise<void> {
    ...
    const hashtagMap = await this.hashtagRepository.findOrCreateHashtags(hashtagNames);
  ...
  /**
    ...
    */
  async getEchoesByHashtag(hashtagName: string, page: number = 1, limit: number = 20) {
    const result = await this.hashtagRepository.getEchoesByHashtag(hashtagName, page, limit);

    if (result.echoes.length === 0) {
      throw new NotFoundException(`No echoes found for hashtag #${hashtagName}`);
    }

    //* Enrich with engagement counts
    const echoIds = result.echoes.map(echo => echo.id);
    const engagementCounts = await this.engagementCountRepository.getCountsForEchoes(echoIds);
  ...
  /**
    ...
    * @param hashtagName 
    * @returns //? Detailed statistics for hashtag analytics
    */
  async getHashtagStats(hashtagName: string) {
    const stats = await this.hashtagRepository.getHashtagStats(hashtagName);

    if (!stats) {
      throw new NotFoundException(`Hashtag #${hashtagName} not found`);
    }
    return stats;
  }
