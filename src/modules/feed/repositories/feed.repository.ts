...
export class FeedRepository {
  ...
  async getTimelineEchoes(
    ...
    try {
      ...
      // Include user's own echoes in timeline
        ...
        include: {
          ...
          author: {
            select: {
              id: true,
              username: true,
              avatar: true, // Only username and avatar, no displayName
            },
          },
          _count: {
            select: {
  ...
  async getTrendingEchoes(cursor: Date | null, limit: number) {
    try {
      ...
      const echoesWithScores = echoes.map(echo => ({
        ...echo,
        trendingScore: this.calculateTrendingScore(echo, echo._count),
      }));
  ...
  private calculateTrendingScore(echo: any, counts: any): number {
    ...
    // Time decay factor
    const now = new Date();
    const hoursOld = (now.getTime() - echo.createdAt.getTime()) / (1000 * 60 * 60);
